...
- [x] 01-02-PLAN.md — MCP server and HTTP action validation driven by the registry; stdio smoke test (wave 2)
- [x] 01-03-PLAN.md — The method text written once under `method/`, rendered to five targets, CI failing on drift (wave 2)
...
- [x] 01-06-PLAN.md — The `turns` and `usage` tables: honest per-request usage for every turn (wave 5)

**Wave 6** *(blocked on Wave 5 completion)*

- [x] 01-07-PLAN.md — Loopback bind, Host and Origin checks, per-install token, one version string (wave 6)
...
- [x] 01-09-PLAN.md — The document routes guarded like the API they unlock: no token in the markup, an HttpOnly derived session cookie, and a Host check bound to the address the request arrived on (gap wave 1)

**Gap wave 2** *(blocked on gap wave 1 completion)*

- [x] 01-10-PLAN.md — `assertPublicHost` in `fromGitClone` before the scratch directory and the git spawn, with the private-address case `guards.test.ts` never had, and the new import ring written down at both ends of itself (gap wave 2)

**Gap wave 3** *(blocked on gap wave 2 completion)*

- [x] 01-11-PLAN.md — `deleteLesson` and `deleteLearner` clear the `turns` and `usage` rows their own comments promise (WR-01) (gap wave 3)
...
- [x] 01-13-PLAN.md — The repo importer reads nothing outside the tree it was given: `lstatSync` in the walker and the read loop, a `realpathSync` root pin for the `git ls-files` path, `isSecretName` on the tarball path too, and a clone pinned to the URL the guard judged (gap wave 5)

**Gap wave 6** *(blocked on gap wave 5 completion)*

- [x] 01-14-PLAN.md — The loopback credential posture decided and then stated: no fail-open on an unknown local address, the browser's credential written down instead of inherited, and three sentences that each map to a case or a line (gap wave 6)
...
**Gap closure, third round** *(from the third re-verification of 01-VERIFICATION.md, status gaps_found at 3/5 — run with `/gsd-execute-phase 1 --gaps-only`)*

**Gap wave 8**

- [x] 01-16-PLAN.md — A suite that passes on its first run and a `PORT` the server can actually be addressed on: one OS-allocated spawn helper for all three server-spawning suites, and a validated `PORT` that refuses rather than 403ing every request (gap wave 8)

**Gap wave 9** *(blocked on gap wave 8 completion)*

- [x] 01-17-PLAN.md — A repo import that runs no code the repository carries: the `git ls-files` argv pinned the way the clone argv was, the verifier's own end-to-end reproduction as a case, an offline battery over three config knobs, and the confinement sentence saying which of its two halves it means (gap wave 9)

**Gap wave 10** *(blocked on gap wave 9 completion)*

- [x] 01-18-PLAN.md — A ledger that reconciles on an upgraded install and across a crash: migration 4 backfilling the honest blank for every ended turn the ledger never saw, the invariant made precise, and `endTurn` closing the turn and its usage row in one transaction (gap wave 10)
...
**Gap closure, fourth round** *(from the fourth re-verification of 01-VERIFICATION.md, status gaps_found at 4/5 — run with `/gsd-execute-phase 1 --gaps-only`)*

**Gap wave 12**

- [x] 01-20-PLAN.md — The advertised revocation made real: the install token read live behind a one-second cache, both credential middlewares and the cookie mint reading it instead of a boot snapshot, a deletion that stays a deletion, and the case named for revocation rewritten to actually revoke (gap wave 12)

**Gap wave 13** *(blocked on gap wave 12 completion)*

- [x] 01-21-PLAN.md — `repo.ts`'s security rationale describing mitigations that exist: the decided pins added to the clone argv, the sentence made accurate in both directions, and a case that goes red the next time the comment and the argv drift apart (gap wave 13)
...
|-------|----------------|--------|-----------|
| 1. Foundation | 21/21 | In Progress|  |
