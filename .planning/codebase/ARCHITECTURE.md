...
- **Circular imports:** `events.ts` → `export.ts` → `db.ts`, and `actions.ts` ↔ `index.ts` are acyclic; `agent.ts` re-exports from `prompts.ts`/`notices.ts`/`tools.ts` for `index.ts`. `db.ts` imports `schedule.ts` and `schedule.ts` imports only a type from `db.ts` (type-only, safe). `repo.ts` → `library.ts` → `materials.ts` → `repo.ts` is a real ring, safe only because no module in it reads another's bindings at module-evaluation time: `library.ts` uses `partsOf`/`SEP`/`titleOf` only inside function bodies and `materials.ts` calls `collectRepo` only inside `attachRepo`, so moving any of them to module scope breaks `repo.ts`'s import.
