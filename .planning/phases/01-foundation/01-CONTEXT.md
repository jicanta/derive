...
- **D-09:** The web app gets the per-install token **injected at serve time**: in production the server injects it into the `index.html` it serves; in development `web/vite.config.ts` reads `~/.derive/token` at startup and sets the header on its `/api` proxy. **No endpoint ever hands the token out** — there is deliberately nothing for a hostile page to call. A missing or unreadable token file must fail with a clear message, not silently.
  ...
  - **Mechanism superseded by plan 01-09 (gap closure), intent strengthened — this decision's text above is left verbatim.** Verification found the injection step of D-09 was itself the disclosure: `curl http://127.0.0.1:PORT/` with no headers returned the token in full, because the document routes ran no Host, Origin or credential check. Plan `01-09-PLAN.md` deletes the injection and hands the browser a *derived* credential instead — an HttpOnly, `SameSite=Strict` `derive_session` cookie whose value is `HMAC-SHA256(token, "derive browser session v1")`, set only on a document response that has already passed the Host and Origin checks. D-09's two binding commitments hold harder than before: **no endpoint ever hands the token out** (it now never leaves the server process at all), and a missing or unreadable token file still fails loudly. What changed is only *what* the browser is handed. The dev-proxy half of D-09 — `web/vite.config.ts` reading `~/.derive/token` and setting the header — is unchanged. Read D-09 together with this note; the mechanism sentence alone is stale.
