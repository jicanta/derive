...
argument-hint: <topic you want to actually understand> [paths to course slides, PDFs or notes]
allowed-tools: mcp__plugin_derive_derive__start_lesson, mcp__plugin_derive_derive__attach_material, mcp__plugin_derive_derive__read_material, mcp__plugin_derive_derive__search_material, mcp__plugin_derive_derive__quiz, mcp__plugin_derive_derive__ask, mcp__plugin_derive_derive__set_plan, mcp__plugin_derive_derive__node_status, mcp__plugin_derive_derive__set_phase, mcp__plugin_derive_derive__explain_back, mcp__plugin_derive_derive__remember, mcp__plugin_derive_derive__learner_profile, mcp__plugin_derive_derive__end_lesson, WebSearch, WebFetch, Skill
---
...
2. Call the derive `start_lesson` tool with the topic. If the argument names files or folders (course slides, a PDF, lecture notes: .pdf, .pptx, .docx, .md, .txt), pass them as `files`; the tool uploads them and returns a brief of the material (its outline, or its full text when short) with instructions on using it. `start_lesson` also opens the companion page in the browser and returns what is already known about this learner. Read both before probing: build on locked nodes, re-derive shaky ones, watch for listed misconceptions, and when there is material, read the relevant pages with `read_material` before you plan so the plan covers what the course covers, in its notation. If the learner mentions files later, `attach_material` adds them mid-lesson.
