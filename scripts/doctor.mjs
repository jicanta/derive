...
// How everything authenticates: the MCP server, the plugin hook and the Vite dev proxy read this file and send it as the x-derive-token header,
// and the browser presents it once in the URL of its first request, then holds a separate derived cookie the page itself cannot read.
// Anyone who can read this file can drive the learner's lessons and read anything Derive can read.
...
if (!existsSync(tokenFile)) warn('Install token', `${tokenFile} does not exist yet`, 'Start Derive once (`pnpm start`) and it makes one.');
  ...
  else fail('Install token', `${tokenFile} is mode 0${mode}, not 0600`, `Run \`chmod 600 ${tokenFile}\`. That file is how the MCP server, the plugin hook, the dev proxy and your browser's first request all authenticate; anyone who can read it can drive your lessons and read anything Derive can read.`);
}
