...
function gitListFiles(dir: string): string[] | null {
  ...
  try {
    // The directory being listed is attacker-controlled data, and `ls-files --others` resolves that directory's own .git/config: core.fsmonitor there is a command git spawns as this process, before any realpath or lstat check in fromDirectory has a chance to run -- a guard downstream of the spawn is not a guard. Command-line `-c` is the one layer of git's config precedence that outranks a repository's own file, which is why the pins ride on the argv rather than in a check. The transport and credential knobs (core.sshCommand, credential.helper, protocol.*) are not on this argv because a listing opens no transport and is never asked for a credential; they are on the clone argv in fromGitClone below, which is the path that does open one, and that argv now carries this one's three spawn knobs as well, so the two git invocations in this file have one machine-config posture rather than two. Three knobs are pinned on neither argv, and that is said here rather than left to silence, because a sentence that lists only what is handled reads as coverage: check-in content filters (filter.*.clean), an external diff driver (diff.external) and the pack-objects hook (uploadpack.packObjectsHook). Neither path filters content on check-in, neither runs a diff and neither serves a pack, so neither is reachable today -- and whoever makes one of them reachable reads here that it is unhandled rather than that it is taken care of. GIT_CONFIG_NOSYSTEM drops the machine's system config on both paths; the learner's own global config is left alone here, because it is not attacker-controlled data and it carries the excludes file this listing exists to honour.
...
async function fromGitClone(url: string): Promise<RepoSource> {
  ...
  try {
    execFileSync('git', ['-c', 'http.followRedirects=false', '-c', 'protocol.allow=never', '-c', 'protocol.https.allow=always', '-c', 'core.sshCommand=false', '-c', 'credential.helper=', '-c', 'core.fsmonitor=false', '-c', 'core.hooksPath=/dev/null', '-c', 'core.pager=cat', 'clone', '--depth', '1', '--quiet', url, tmp], { stdio: ['ignore', 'ignore', 'pipe'], timeout: 120_000, env: { ...process.env, GIT_CONFIG_NOSYSTEM: '1', GIT_TERMINAL_PROMPT: '0' } });
  } catch (e) {
